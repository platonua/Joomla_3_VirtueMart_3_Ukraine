...
class plgVmPaymentPlaton extends vmPSPlugin
{
    ...
    function __construct(&$subject, $config)
    {
        ...
        $this->_tablepkey  = 'id'; //virtuemart_PLATON_id';
        $this->_tableId    = 'id'; //'virtuemart_PLATON_id';
    ...
    {
        return $this->createTableSQL('Payment PLATON Table');
    }

    function plgVmOnPaymentNotification()
    {
        $callbackParams = JRequest::get('post');
        $log            = JPATH_SITE.'/logs/platon-callback.log';
        file_put_contents($log, date('Y-m-d H:i:s').": Params: ".print_r($callbackParams, true)."\n\n", FILE_APPEND);


        if (!array_key_exists('order', $callbackParams) || !isset($callbackParams['order'])) {
        ...
        $virtuemart_order_id = $modelOrder->getOrderIdByOrderNumber($callbackParams['order']);
        ...
        switch ($callbackParams['status']) {
            ...
            case 'REFUND':
                ...
                $modelOrder->updateStatusForOneOrder($virtuemart_order_id, $data, true);
                file_put_contents($log, date('Y-m-d H:i:s').": Success refund\n", FILE_APPEND);
    ...
    /**
     * Prepare data and redirect to Platon payment platform
     *
    ...
    function plgVmConfirmedOrder($cart, $order)
    {
        ...
        $fields['order'] = $order['details']['BT']->order_number;
        ...
        $fields['error_url'] = $uri2->toString();
        ...
        $fields['data'] = base64_encode(
            json_encode(
                array(

                    'amount'   => sprintf("%01.2f", $order['details']['BT']->order_total),
                    'name'     => JText::_(
    ...
    {
        vmdebug('platon plgVmOnCheckAutomaticSelectedPayment');
